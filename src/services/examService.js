...
export const fetchUnFinishedExams = async ({ page = 1, size = 3 }) => {
  let examlist = await callApi({ path: GET_UNFINISHED_EXAMS_ENDPOINT, params: { page, size } });
...
export const markAsFavourite = async (examId) => {
  return await callApi({ path: MARK_AS_FAVOURITE_ENDPOINT, method: 'POST', params: { examId } });
};
